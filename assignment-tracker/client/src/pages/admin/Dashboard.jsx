...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    api.admin.dashboard(p).then(setData).catch(() => {});
    ...
    <div style={{ minHeight: "100vh", background: "#f8f9fa", fontFamily: "'Inter', sans-serif" }}>
      <div style={{ background: "#fff", borderBottom: "1px solid #e9ecef", padding: "12px 0" }}>
        <div className="container d-flex justify-content-between align-items-center">
          <div>
            <h5 style={{ fontWeight: 700, margin: 0, color: "#212529" }}>Admin Dashboard</h5>
      ...
      <div className="container py-4">
        <div className="row g-3 mb-4">
          ...
          <div className="col-md-3">
            <div onClick={() => viewTasks("Submitted")} style={{ cursor: "pointer", textDecoration: "none" }}>
          ...
          <div className="col-md-3">
            <div onClick={() => viewTasks("Pending")} style={{ cursor: "pointer", textDecoration: "none" }}>
          ...
          <div className="col-md-3">
            <div onClick={() => viewTasks("Late")} style={{ cursor: "pointer", textDecoration: "none" }}>
        ...
        <div className="d-flex flex-wrap gap-2 mb-4">
          <Link to="/admin/config" className="btn btn-primary">Department Management</Link>
          <Link to="/admin/users" className="btn btn-success">User Management</Link>
          <Link to="/admin/promote" className="btn btn-info text-white">Student Promotion</Link>
          <Link to="/admin/passedout" className="btn btn-secondary">Passed Out Students</Link>
          <Link to="/admin/calendar" className="btn btn-primary">Assignment Calendar</Link>
        </div>
                ...
                <table className="table table-hover mb-0" style={{ fontSize: 14 }}>
                  ...
                  <tbody>
                    {data.displayTasks.map((t, i) => (
                      <tr key={i}>
                        <td style={{ padding: "12px 16px" }}>{t.student?.name} ({t.student?.username})</td>
                        <td style={{ padding: "12px 16px" }}><Link to={`/admin/view/${t.assignment.id}`} style={{ color: "#4d96ff", textDecoration: "none" }}>{t.assignment.title}</Link></td>
                    ...
                    {data.displayTasks.length === 0 && (
                      <tr><td colSpan={6} className="text-center py-4" style={{ color: "#999" }}>No tasks found</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
                ...
                <table className="table table-hover mb-0" style={{ fontSize: 14 }}>
                  <thead style={{ background: "#f8f9fa" }}>
                    <tr>
                      ...
                      <th style={{ padding: "12px 16px", fontWeight: 600 }}>Assigned By</th>
                      <th style={{ padding: "12px 16px", fontWeight: 600 }}>Department</th>
                  ...
                  <tbody>
                    {data.assignments.length === 0 ? (
                      ...
                      data.assignments.map(a => (
                        <tr key={a.id}>
                          <td style={{ padding: "12px 16px" }}><Link to={`/admin/view/${a.id}`} style={{ color: "#4d96ff", textDecoration: "none", fontWeight: 500 }}>{a.title}</Link></td>
                          <td style={{ padding: "12px 16px" }}>{a.assignedBy || "-"}</td>
