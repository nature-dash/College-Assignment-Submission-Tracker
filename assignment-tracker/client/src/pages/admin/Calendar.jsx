...
const statusColors = {
  submitted: "#2ecc71", pending: "#f5a623", late: "#e74c3c", checked: "#4d96ff",
};
...
export default function Calendar() {
  ...
  useEffect(() => {
    api.admin.calendar().then(setData).catch(() => {});
  }, []);
  ...
  return (
    <div style={s.page}>
      <div style={s.header}>
        <span style={s.headerTitle}>Assessment Calendar</span>
      ...
      <div style={s.content}>
        <div className="container-fluid">
          <div style={s.card}>
            <h6 style={{ fontWeight: 700, fontSize: 16, color: "#1a1c2e", marginBottom: 16 }}>Assessment Calendar</h6>
                ...
                return (
                  <div key={d} onClick={() => setSelectedDay(isSelected ? null : d)} style={{
                    ...
                    {dayAssignments.slice(0, 3).map((a, j) => (
                      <Link key={j} to={`/admin/view/${a.id}`} style={{
                        fontSize: 10, color: statusColors.pending, textDecoration: "none", lineHeight: 1.3, marginBottom: 1,
            ...
            <div style={s.card}>
              ...
              {selectedAssignments.map((a, i) => (
                <div key={i} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "12px 0", borderBottom: i < selectedAssignments.length - 1 ? "1px solid #f0f0f0" : "none" }}>
                  <div>
                    <Link to={`/admin/view/${a.id}`} style={{ color: "#4d96ff", textDecoration: "none", fontWeight: 600, fontSize: 14 }}>{a.title}</Link>
